...
export async function register(input: RegisterInput): Promise<{ user: AuthUser; tokens: TokenPair }> {
  ...
  return { user, tokens: buildTokenPair(user.id, user.email) };
}
...
export async function login(input: LoginInput): Promise<{ user: AuthUser; tokens: TokenPair }> {
  ...
  // Constant-time comparison even on "user not found" to prevent timing attacks
...
export async function refreshTokens(input: RefreshInput): Promise<TokenPair> {
  ...
  // Blocklist the used refresh token immediately — rotating tokens prevent replay attacks
  const decoded = jwt.decode(input.refreshToken) as { exp?: number };
  const ttl = decoded.exp ? decoded.exp - Math.floor(Date.now() / 1000) : 7 * 24 * 3600;
  if (ttl > 0) {
    await redis.setex(`blocklist:${payload.jti}`, ttl, '1');
  }

  return buildTokenPair(payload.sub, payload.email);
}
...
function buildTokenPair(userId: string, email: string): TokenPair {
  return {
